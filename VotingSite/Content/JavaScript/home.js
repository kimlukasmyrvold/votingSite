// Open the vote form modal and add data to it
async function openVoteForm(data) {
    data.preventDefault();
    const partiID = data.target.dataset.id;

    // Show the logo for the selected "parti"
    const partiLogo = document.querySelector("#vote_form .partiLogo img");
    partiLogo.setAttribute("src", `Content/Images/PartyLogos/${(await getPartiInfo(partiID)).name}.png`);

    // Show the name for the selected "parti"
    const votingSelected = document.querySelector("#vote_form .partiName");
    votingSelected.textContent = (await getPartiInfo(partiID)).fullName;
    votingSelected.style.setProperty("--parti-color", (await getPartiInfo(partiID)).color);

    // Show fnum input and kommune select if a fylke is selected
    showRemains();
...
// Show kommune select list and fnum input if a fylke is selected
function showRemains() {
    ...
    ajax("Default.aspx/GetFromKommuner", data, addKommunerToSelectList);
}

function addKommunerToSelectList(response) {
...
function getPartiInfo(key) {
    return fetch('/Content/JSON/partier.json')
        ...
        .catch(e => {
            console.error("There was an error fetching partier.json:", e)
        });
}
...
function reorderPartiItems() {
    ...
    partierItems.sort((a, b) => {
        ...
        if (orderMapping[sideA] < orderMapping[sideB]) return -1;
        if (orderMapping[sideA] > orderMapping[sideB]) return 1;
        return 0;
    });
